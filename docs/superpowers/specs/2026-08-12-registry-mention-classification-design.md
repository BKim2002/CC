...
### 프로덕션 규모 검증 (구현 1단계에서 확인, 이 절은 사후 갱신)

`tests/test_competency_query.py::_snapshot()`이 정식명 **62개**를 담고 있어 프로덕션 규모(~52개)와 같다. 초기 판단과 달리 별도 프로덕션 스냅샷 없이 검증할 수 있었다.

여기서 **처음 세웠던 속성 명제가 틀렸음이 드러났다.** "서로 다른 정식명 두 개가 cutoff 안에 들어오면 실패"는 이 레지스트리에서 **417쌍이 위반**한다. `공감성 하위 1 / 2 / 3`(0.88), `답변태도 세부항목 1 / 2`(0.91)처럼 형제 이름이 설계상 거의 동일하기 때문이다.

이 충돌은 무해하다. 근접 매칭은 **해석에 실패한 mention에만** 돌고, 정확히 입력된 라벨은 `snapshot.lookup`에서 바로 해석되어 근접 매칭에 도달하지 않는다.

올바른 명제는 재현율이다: **한 음절 오타가 자기 이름을 후보 안에서 되찾는가.**

측정 결과 62개 정식명 전부에 대해 **62/62가 1순위로 복원**됐다. 형제 충돌이 많아도 한 음절 오타는 형제보다 원본에 훨씬 가깝다.

cutoff 0.6은 프로덕션 규모에서 검증되었다.
...
| 속성 | 62개 정식명 각각의 한 음절 오타가 자기 이름을 후보 안에서 되찾음 |

속성 테스트는 62개 이름 픽스처 전체를 돈다. 최소 50개를 검사했는지도 함께 단언해 픽스처가 축소되면 조용히 무력화되지 않게 한다.
...
4. 62개 정식명 각각의 한 음절 오타가 자기 이름을 후보 안에서 되찾는다. *(1단계에서 62/62 1순위로 충족)*
5. 기존 테스트 874건이 통과한다.
