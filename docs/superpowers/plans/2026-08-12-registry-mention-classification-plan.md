...
- **속성 테스트:** 62개 정식명 각각의 한 음절 오타가 자기 이름을 후보 안에서 되찾음
- 스위트: 874 + 신규, 기존 테스트 변경 없음

**✅ 완료 — 891 passed (874 + 17).**

계획 작성 시 세웠던 속성 명제("서로 다른 정식명이 cutoff 안에 들어오면 실패")는 **틀렸다.** `tests/test_competency_query.py::_snapshot()`의 62개 이름에서 417쌍이 위반한다 — `공감성 하위 1/2/3`(0.88)처럼 형제 이름이 설계상 거의 동일하기 때문이다. 이 충돌은 무해하다: 정확히 입력된 라벨은 `snapshot.lookup`에서 해석되어 근접 매칭에 도달하지 않는다.

재현율 명제로 교체했고, 62/62가 1순위로 복원됨을 확인했다. 이 픽스처가 프로덕션 규모(~52개)라 **아래 "릴리스 전 별도 확인" 절은 해소되었다.**
...
계획 작성 시에는 합성 레지스트리 이름이 6개뿐이라 cutoff를 프로덕션 규모에서 검증할 수 없다고 봤다. 실제로는 `tests/test_competency_query.py::_snapshot()`이 정식명 **62개**를 담고 있어 프로덕션(~52개)과 같은 규모다.

1단계에서 62/62 재현율을 확인했으므로 별도 프로덕션 실행은 필요 없다.
